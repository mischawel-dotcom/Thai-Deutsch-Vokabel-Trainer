...
export async function ensureProgressForSentenceEntries(entryIds: number[]) {
  const uniqueIds = Array.from(new Set(entryIds.filter((id) => Number.isFinite(id) && id > 0)));
  if (uniqueIds.length === 0) return;

  const table = db.table<SrsProgress, number>("sentencesProgress");
  const existing = await table.bulkGet(uniqueIds);
  const now = Date.now();
...
export async function gradeSentenceCard(entryId: number, grade: Grade) {
  const table = db.table<SrsProgress, number>("sentencesProgress");
  const prev = await table.get(entryId);
  const next = nextProgress(prev, grade);
  next.entryId = entryId;
